...
rl.question('Enter the gameId: ', (gameId) => {
    ...
    function scrapeWebsite() {
        ...
        request.get(url).then((html) => {
            ...
            const homeShots = data[0];
            const guestShots = data[1];
            const homeSaves = data[2];
            const guestSaves = data[3];
            const homePIM = data[4];
            const guestPIM = data[5];
            const homePP = data[6];
            const guestPP = data[7];
            // Check if the values have changed before creating a new image
            if (homeShots !== previousHomeShots ||
